  ...
  doPayment = async () => {
    ...
    fetch('https://us-central1-hotelbooking-e4531.cloudfunctions.net/payWithStripe', {
      ...
      .then((response) => {
        ...
        addTransaction(this.state.email, this.state.success, this.state.price, this.state.service_name, this.state.service_type, this.state.id, this.state.token);
      })
      .catch((error) => {
        console.error(error);
      });;
        ...
        <View
          ...
          {success &&
            <>
              ...
              <Button onPress={() => this.props.navigation.navigate('TransactionListScreen')}
